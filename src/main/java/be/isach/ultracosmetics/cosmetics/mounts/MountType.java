...
public enum MountType {
    ...
    SKYSQUID("ultracosmetics.mounts.skysquid", "SkySquid", Material.INK_SACK, (byte) 0, EntityType.SQUID, "&7&oWat.", Main.getInstance().getMounts().getSquidClass()),
    SLIME("ultracosmetics.mounts.slime", "Slime", Material.SLIME_BALL, (byte) 0, EntityType.SLIME, "&7&oSplat! Splat!", Main.getInstance().getMounts().getSlimeClass()),
    ...
    SPIDER("ultracosmetics.mounts.spider", "Spider", Material.WEB, (byte) 0, EntityType.SPIDER, "&7&oYOU are the spider jockey!", Main.getInstance().getMounts().getSpiderClass()),
    ...
    public boolean isEnabled() {
        if ((this == FLYINGSHIP || this == SKYSQUID) && (Main.getServerVersion().compareTo(ServerVersion.v1_9_R1) >= 0))
            return false;
    ...
    public Mount equip(Player player, Main ultraCosmetics) {
        Mount mount = null;
        try {
            mount = clazz.getDeclaredConstructor(UUID.class, Main.class).newInstance(player == null ? null : player.getUniqueId(), ultraCosmetics);
            mount.equip();
