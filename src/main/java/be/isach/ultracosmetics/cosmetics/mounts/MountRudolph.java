...
public class MountRudolph extends Mount {
    ...
    public MountRudolph(UUID owner, Main ultraCosmetics) {
    ...
    protected void onEquip() {
        if (owner != null) {
            ...
            Main.getInstance().getEntityUtil().setHorseSpeed(horse, 0.4d);
    ...
    private ArmorStand spawnArmorStand(boolean right) {
        ...
        armorStand.setMetadata("C_AD_ArmorStand", new FixedMetadataValue(Main.getInstance(), getPlayer().getUniqueId().toString()));
        return armorStand;
    }
        ...
        new Thread() {
            ...
            public void run() {
                for(Player player : getPlayer().getWorld().getPlayers()) {
                    Main.getInstance().getEntityUtil().sendTeleportPacket(player , right);
                    Main.getInstance().getEntityUtil().sendTeleportPacket(player , left);
                }
            }
        }.start();
    }
