...
public abstract class ParticleEffect implements Listener {
    ...
    public ParticleEffect(final UUID owner, final ParticleEffectType type) {
        ...
        if (owner != null) {
            ...
            if (Main.getUltraPlayer(getPlayer()).currentParticleEffect != null)
                Main.getUltraPlayer(getPlayer()).removeParticleEffect();
            BukkitRunnable runnable = new BukkitRunnable() {
                @Override
                public void run() {
                    try {
                        if (Bukkit.getPlayer(owner) != null
                                && Main.getUltraPlayer(Bukkit.getPlayer(owner)).currentParticleEffect != null
                                && Main.getUltraPlayer(Bukkit.getPlayer(owner)).currentParticleEffect.getType() == type) {
            ...
            runnable.runTaskTimerAsynchronously(Main.getInstance(), 0, type.getRepeatDelay());
            listener = new ParticleEffectListener(this);
            getPlayer().sendMessage(MessageManager.getMessage("Particle-Effects.Summon").replace("%effectname%", (Main.getInstance().placeholdersHaveColor()) ? type.getName() : Main.filterColor(type.getName())));
            Main.getUltraPlayer(getPlayer()).currentParticleEffect = this;
        }
    }
     ...
     */
    public void clear() {
        Main.getUltraPlayer(getPlayer()).currentParticleEffect = null;
        try {
        ...
        if (getPlayer() != null)
            getPlayer().sendMessage(MessageManager.getMessage("Particle-Effects.Unsummon").replace("%mountname%", (Main.getInstance().placeholdersHaveColor()) ? type.getName() : Main.filterColor(type.getName())));
        owner = null;
    }
