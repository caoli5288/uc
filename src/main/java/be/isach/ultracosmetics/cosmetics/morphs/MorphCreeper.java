    ...
    public MorphCreeper(UUID owner) {
        ...
        if (owner != null) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    if (getPlayer() == null
                            || Main.getUltraPlayer(getPlayer()).currentMorph != creeper) {
                        cancel();
                        return;
                    }
...
//                            disguise.setShowName(true);
                        ...
                        if (charge == 100) {
                            ...
                            Main.getInstance().getActionBarUtil().sendActionMessage(getPlayer(), "");
                            charge = 0;
                            return;
                        }
                        if (charge > 0)
                    ...
                    if (charge > 0 && charge < 100) {
                        if (charge < 5) {
                            ...
                            Main.getInstance().getActionBarUtil().sendActionMessage(getPlayer(), MessageManager.getMessage("Morphs.Creeper.charging").replace("%chargelevel%", charge + ""));
                    } else if (charge == 100)
                        Main.getInstance().getActionBarUtil().sendActionMessage(getPlayer(), MessageManager.getMessage("Morphs.Creeper.release-to-explode"));
