    ...
    public MorphChicken(UUID owner) {
        ...
        if (owner != null) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    if (getPlayer() == null
                            || Main.getUltraPlayer(getPlayer()).currentMorph != chicken) {
                        cancel();
                        return;
                    }

                    Main.getInstance().getEntityUtil().chickenFall(getPlayer());
                        ...
                        public void run() {
                            try {
                                for (Chicken chicken : chickens) {
                                    Main.getInstance().getEntityUtil().follow(getPlayer(), chicken);
                                }
                            } catch (Exception exc) {
                            }
                        }
                    };
                    followRunnable.runTaskTimer(Main.getInstance(), 0, 1);
                    Bukkit.getScheduler().runTaskLater(Main.getInstance(), new Runnable() {
                        @Override
                        public void run() {
                            for (Chicken chicken : chickens) {
