...
public class MorphBlaze extends Morph {
    ...
    public MorphBlaze(UUID owner) {
        ...
        if (owner != null) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    if (getPlayer() == null
                            || Main.getUltraPlayer(getPlayer()).currentMorph != blaze) {
                        cancel();
                        return;
                    }
    ...
    public void onKick(PlayerKickEvent event) {
        if(event.getPlayer() == getPlayer() && Main.getUltraPlayer(getPlayer()).currentMorph == this && event.getReason().contains("Flying"))
