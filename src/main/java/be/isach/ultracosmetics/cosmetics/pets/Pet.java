...
public abstract class Pet implements Listener {
     ...
     */
    public void equip() {
        this.followTask = Main.getInstance().newPlayerFollower(this, getPlayer());
        if (Main.getUltraPlayer(getPlayer()).currentPet != null)
            Main.getUltraPlayer(getPlayer()).removePet();
        Main.getUltraPlayer(getPlayer()).currentPet = this;
        ...
        if (Main.getUltraPlayer(getPlayer()).getPetName(getType().getConfigName()) != null)
            armorStand.setCustomName(Main.getUltraPlayer(getPlayer()).getPetName(getType().getConfigName()));
            ...
            public void run() {
                try {
                    if (!entity.isValid()) {
                        ...
                        entity.remove();
                        if (getPlayer() != null)
                            Main.getUltraPlayer(getPlayer()).currentPet = null;
                        for (Item i : items)
                            i.remove();
                        items.clear();
                        try {
                    ...
                    if (Bukkit.getPlayer(owner) != null
                            && Main.getUltraPlayer(Bukkit.getPlayer(owner)).currentPet != null
                            && Main.getUltraPlayer(Bukkit.getPlayer(owner)).currentPet.getType() == type) {
        ...
        runnable.runTaskTimer(Main.getInstance(), 0, 3);
        ...
        Main.getInstance().getPathfinderUtil().removePathFinders(entity);
...
//        this.entity.setPassenger(armorStand);
        if (getType() == PetType.WITHER) {
            ...
            if (Main.getUltraPlayer(getPlayer()).getPetName(getType().getConfigName()) != null)
                this.entity.setCustomName(Main.getUltraPlayer(getPlayer()).getPetName(getType().getConfigName()));
            armorStand.remove();
        }
        this.entity.setMetadata("Pet", new FixedMetadataValue(Main.getInstance(), "UltraCosmetics"));

        getPlayer().sendMessage(MessageManager.getMessage("Pets.Spawn").replace("%petname%", (Main.getInstance().placeholdersHaveColor())
                ? getType().getMenuName() : Main.filterColor(getType().getMenuName())));
    }
    ...
    public void clear() {
        if (armorStand != null)
            armorStand.remove();
        removeEntity();
        if (getPlayer() != null && Main.getUltraPlayer(getPlayer()) != null)
            Main.getUltraPlayer(getPlayer()).currentPet = null;
        for (Item i : items)
            i.remove();
        items.clear();
        ...
        if (getPlayer() != null) {
            getPlayer().sendMessage(MessageManager.getMessage("Pets.Despawn").replace("%petname%", (Main.getInstance().placeholdersHaveColor())
                    ? getType().getMenuName() : Main.filterColor(getType().getMenuName())));
            Main.getUltraPlayer(getPlayer()).currentPet = null;
        }
        owner = null;
