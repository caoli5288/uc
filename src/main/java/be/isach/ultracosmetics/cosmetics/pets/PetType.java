...
public enum PetType {
    ...
    PUMPLING("ultracosmetics.pets.pumpling", "Pumpling", Material.PUMPKIN, (byte) 0, "&7&oJust a little floating pumpkin", EntityType.ZOMBIE, Main.getInstance().getPets().getPumplingClass()),
    ...
    PetType(String permission, String configName, Material material, byte data, String defaultDesc, EntityType entityType, Class<? extends Pet> clazz) {
        ...
        if (SettingsManager.getConfig().get(new StringBuilder().append("Pets.").append(configName).append(".Description").toString()) == null) {
            ...
            Main.config.addDefault(new StringBuilder().append("Pets.").append(configName).append(".Description").toString(), getDescriptionColored(), "description of this pet.");
        } else {
