...
public class GadgetBlizzardBlaster extends Gadget {
    ...
    void onRightClick() {
        ...
        final int i = Bukkit.getScheduler().runTaskTimerAsynchronously(Main.getInstance(), new BukkitRunnable() {
            @Override
            public void run() {
                if (Main.getUltraPlayer(getPlayer()).currentGadget != instance) {
                    cancel();
                    return;
                }
                ...
                for (int i = 0; i < 3; i++) {
                    Main.getInstance().getEntityUtil().sendBlizzard(getPlayer(), loc, affectPlayers, v);
                }
                loc.add(v);
            }
        }, 0, 1).getTaskId();
    ...
    @Override
    public void onClear() {
        Main.getInstance().getEntityUtil().clearBlizzard(getPlayer());
