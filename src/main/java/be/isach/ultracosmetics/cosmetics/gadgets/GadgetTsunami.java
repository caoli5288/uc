...
public class GadgetTsunami extends Gadget {
    ...
    void onRightClick() {
        ...
        final int i = Bukkit.getScheduler().runTaskTimerAsynchronously(Main.getInstance(), new Runnable() {
            @Override
            public void run() {
                ...
                if (affectPlayers)
                    for (final Entity ent : getPlayer().getWorld().getEntities()) {
                        if (ent.getLocation().distance(loc) < 0.6 &&
                            ...
                            Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> cooldownJump.remove(ent), 20);
                        }
                    }
        ...
        Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> Bukkit.getScheduler().cancelTask(i), 40);
