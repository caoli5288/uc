                    ...
                    public void run() {
                        if (i.isValid()) {
                            if (Main.getInstance().isAmmoEnabled()) {
                                getPlayer().getInventory().setItem((int) SettingsManager.getConfig().get("Gadget-Slot"), ItemFactory.create(getMaterial(), getData(), "§f§l" + Main.getUltraPlayer(getPlayer()).getAmmo(getType().toString().toLowerCase()) + " " + getName(), "§9Gadget"));
                            } else {
