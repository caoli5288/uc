...
public class GadgetExplosiveSheep extends Gadget {
    ...
    void onRightClick() {
        ...
        Main.getInstance().getEntityUtil().clearPathfinders(s);

        Main.getInstance().explosiveSheep.add(this);
    ...
    @Override
    public void onClear() {
        Main.getInstance().explosiveSheep.remove(this);
        ...
        public void run() {
            ...
            if (time < 0.5) {
                ...
                for (int i = 0; i < 50; i++) {
                    ...
                    Main.getInstance().getEntityUtil().clearPathfinders(sheep);
                    Main.getInstance().getEntityUtil().makePanic(sheep);
                    Bukkit.getScheduler().runTaskLater(Main.getInstance(), new Runnable() {
                        @Override
                        public void run() {
                            UtilParticles.display(Particles.LAVA, sheep.getLocation(), 5);
                            sheep.remove();
                            Main.getInstance().explosiveSheep.remove(gadgetExplosiveSheep);
                        }
                    }, 110);
                }
