...
public class SubCommandGive extends SubCommand {
    ...
    private void addKey(OfflinePlayer p) {
        if (p == null || p.getUniqueId() == null)
            return;
        if (p instanceof Player)
            Main.getPlayerManager().getUltraPlayer((Player) p).addKey();
        else {
            if (Main.getInstance().usingFileStorage())
                SettingsManager.getData(p.getUniqueId()).set("Keys", getKeys(p) + 1);
            else
                Main.db.addKey(Main.index(p));
        }
    }
    ...
    private void addAmmo(GadgetType gadgetType, OfflinePlayer receiver, int ammo) {
        ...
        if (receiver instanceof Player)
            Main.getPlayerManager().getUltraPlayer((Player) receiver).addAmmo(gadgetType.toString().toLowerCase(), ammo);
        else {
            if (Main.getInstance().usingFileStorage())
                ...
                Main.db.addAmmo(Main.index(receiver), gadgetType.toString().toLowerCase(), ammo);
        }
    }

    private int getKeys(OfflinePlayer p) {
        return Main.getInstance().usingFileStorage() ? (int) SettingsManager.getData(p.getUniqueId()).get("Keys") : Main.db.getKey(Main.index(p));
    }
}
